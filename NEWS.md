...
- `tar_hdwgcna()` gains **`prep_path`** (default `"./out/seurat/wgcna_prep.qs2"`), which caches the prepared metacell object on disk. `wgcna_prep` is the memory peak of the whole factory by a wide margin: it holds the entire Seurat object, adds a normalised layer and a dense `scale.data`, and only then reduces to metacells, which on ~50k cells is well into double-digit GB and more than a small server or a memory-capped container has. Everything below it works on metacells and is far cheaper. With `prep_path` set, `wgcna_prep` becomes a `format = "file"` target that returns the path when `hdwgcna_prep_is_valid()` accepts the file, and otherwise builds the object, writes it with `save_seurat()` and returns the path. So the peak is paid once, on a machine that can afford it, and the file is copied to the others. The file wins over the pipeline: an upstream change does not refresh it, precisely so a machine that cannot rebuild it does not try; delete it to force a rebuild. `prep_path = NULL` restores the previous in-store object target. With several `clustering_col` values the scope suffix is inserted before the extension (`out/prep.qs2` becomes `out/prep_0.3.qs2`), since the metacells depend on the clustering column.

- New `hdwgcna_prep_is_valid()`: checks that a file exists, is non-empty, reads back as a Seurat object, carries the named hdWGCNA experiment and has a metacell object attached. It reads the file in full, because a truncated copy is exactly the failure it is meant to catch, and it reports each failure as a message so a broken file leads to a rebuild rather than an obscure error several steps later. Note that it checks structure only: a file prepared with a different `fraction` or `metacell_k` is accepted, so the `tar_hdwgcna()` arguments must match the ones the file was built with.

- New `save_seurat()`, the counterpart of `as_seurat()`, and `seurat_file_formats()` listing the extensions both understand. `tar_hdwgcna()` validates `prep_path`'s extension when the pipeline is DEFINED rather than after the half-hour of work that was supposed to fill it.

- The `wgcna_prep` consumers (`wgcna_<group>_powertest` and `wgcna_<group>`) now read it through `as_seurat()`, so they accept the object or the path and the factory is the same in both modes.
