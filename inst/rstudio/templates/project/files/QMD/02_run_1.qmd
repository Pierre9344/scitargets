...
abstract: |
  ...
  This run is part of the **`r targets::tar_read(parameters_RUN_ID)$project`** project.
params:
  run: RUN_ID
---
...
run_clusters_for_markers = "clusters_0.5" # used as default
if ("singlets_clusters_for_markers" %in% names(tar_read(parameters_RUN_ID))) {
  run_clusters_for_markers = tar_read(parameters_RUN_ID)[["singlets_clusters_for_markers"]]
}
```
...
p <- Seurat::VlnPlot(seurat_obj_RUN_ID_raw,
  features = feat_to_plot,
  ncol = 5, alpha = 0.1, combine = FALSE
)
names(p) <- feat_to_plot
p[["nFeature_RNA"]] <- p[["nFeature_RNA"]] +
  ggplot2::geom_hline(
    yintercept = c(
      tar_read(parameters_RUN_ID)$min_genes_detected,
      ),
    colour = "red", linetype = "dashed", linewidth = 0.5
  ) +
  ggplot2::geom_hline(
    yintercept = c(
      tar_read(parameters_RUN_ID)$max_genes_detected
      ),
    colour = "red", linetype = "dashed", linewidth = 0.5
  )
p[["percent.mt"]] <- p[["percent.mt"]] +
  ggplot2::geom_hline(
    yintercept = tar_read(parameters_RUN_ID)$mt_cutoff,
    colour = "red",
    linetype = "dashed",
    linewidth = 0.5
  )
...
Seurat::Idents(seurat_obj_RUN_ID_singlets) <- run_clusters_for_markers
Seurat::DimPlot(seurat_obj_RUN_ID_singlets, group.by = "HTO_classification", reduction = "pca")
Seurat::DimPlot(seurat_obj_RUN_ID_singlets, group.by = run_clusters_for_markers, reduction = "pca")
...
#| fig-cap: "PCA of the singlets (after features removal)"
#| layout: "[[1,1], [1]]"
#| fig-subcap:
#|   - HTO classification
#|   - Seurat clusters
...
if (tar_exist_objects("seurat_obj_RUN_ID_feat_removed_singlets")) {
  ...
  Seurat::Idents(seurat_obj_RUN_ID_feat_removed_singlets) <- run_clusters_for_markers
  p1 <- Seurat::DimPlot(seurat_obj_RUN_ID_feat_removed_singlets,
    group.by = "HTO_classification", reduction = "pca"
  )
  p2 <- Seurat::DimPlot(seurat_obj_RUN_ID_feat_removed_singlets,
    group.by = run_clusters_for_markers, reduction = "pca"
  )
  p3 <- Seurat::ElbowPlot(seurat_obj_RUN_ID_feat_removed_singlets, ndims = 30)
}
plot(p1)
plot(p2)
plot(p3)
...
FeaturePlotWithCluster <- function(obj = NULL,
  ...
  for (i in seq_along(p)) {
    p[[i]]$data$clusters <- obj@meta.data[[clusters_to_use]]
...
if (tar_exist_objects("markers_RUN_ID")) {
  ...
  for (G in sort(unique(top10$gene))) {
    ...
    p <- FeaturePlotWithCluster(obj, features = G)
    plot(p)
  }
  cat("\n\n:::\n\n")
}
