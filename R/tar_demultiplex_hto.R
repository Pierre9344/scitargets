...
#'   `"worker"` (send them to a `crew` worker). The raw-load step is always pinned
#'   to `"main"` regardless, because Seurat metadata creation bugs on crew workers.
#' @param controller Optional name of a `crew` controller to route the steps to
#'   (via `targets::tar_resources()`), for use with a
...
tar_demultiplex_hto <- function(
  ...
  # Deployment of the demultiplexing steps. All steps run on `deployment`
  # (routed to a named `controller` if given) via the `mt()` wrapper below, EXCEPT
  # the raw-load step, which is pinned to "main" further down: Seurat metadata
  # creation bugs on crew workers.
  ...
  mt <- function(...) {
    targets::tar_target_raw(..., deployment = deployment, resources = movable_resources)
  }
  ...
  hto_demux_steps <- list(
    ...
    targets::tar_target_raw(
      ...
      deployment = "main", # run on main as parallelism seem to bug the metadata creation
