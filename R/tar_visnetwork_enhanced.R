utils::globalVariables(c(
  "type", "from", "to", "description", "has_function",
  "has_stem", "function_parent", "name"
))
...
#'
#' @param keep_report Should the report node be used or not. Default to TRUE
#' @param dataset Character vector that must be present in the targets names to keep them. Default to NA (no filtering). It only affect the nodes generated by tar_demultiplex_hto or the ones names with similar pattern (starting with 'markers_', 'parameters_', 'seurat_obj_', 'cellranger_output_').
...
tar_visnetwork_enhanced <- function(
  ...
  if (!keep_report) {
    nodes <- dplyr::filter(nodes, name != "report")
  }
  # Prepare nodes
  nodes$id <- nodes$name
  ...
  if (!all(is.na(dataset))) {
    if (!is.character(dataset)) {
      stop("dataset must be a character variable.")
    }
